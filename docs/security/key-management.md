...
Envelope 与 verification tag 必须在数据库打开前可用，因此保存于独立 BootstrapStore。接口位于 Security
边界，当前 Proto DataStore 实现位于 Data 层。Room 的 `key_envelopes` 表是未清理的遗留实现，不是当前真相源。
...
5. 锁定时关闭数据库并覆盖内存数组。
6. 删除 Vault 时清除 BootstrapStore 和数据库数据，操作必须显式且不可伪装成错误恢复。
