            ...
            Button(
                onClick = { 
                    if(viewModel.addDialogTitle.isNotBlank() && viewModel.addDialogCategory.isNotBlank()) {
                        encryptMultiple(activity, listOf(viewModel.addDialogUsername, viewModel.addDialogPassword)) { results ->
                            viewModel.addItem(
...
private fun encryptMultiple(activity: FragmentActivity, texts: List<String>, onSuccess: (List<String>) -> Unit) {
    BiometricHelper.authenticate(
        ...
        onSuccess = {
            val results = texts.map { text ->
                CryptoManager.getEncryptCipher()?.let { cipher ->
