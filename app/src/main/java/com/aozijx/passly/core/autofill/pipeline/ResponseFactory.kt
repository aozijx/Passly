 ...
 *
 * 不再依赖 [com.aozijx.passly.domain.model.credential.CredentialCandidate] 或 [com.aozijx.passly.domain.entry.model.VaultEntry]。
 */
@Singleton
class ResponseFactory @Inject constructor() {
    ...
    fun build(context: ResponseContext): InternalFillResponse {
        ...
        if (resolved.isEmpty()) {
            AppTelemetry.w(TAG, "No candidates could be resolved for ${context.parentPackage}")
            return InternalFillResponse()
        }
        AppTelemetry.i(TAG, "Built ${resolved.size} candidates for ${context.parentPackage}")
