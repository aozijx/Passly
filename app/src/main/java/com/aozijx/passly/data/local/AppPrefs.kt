...
class AppPrefs(context: Context) : SecuritySettingsRepository, SystemSettingsRepository,
    ...
    override val isStatusBarAutoHide: Flow<Boolean> =
        appContext.vaultDataStore.data.map { it[AUTO_HIDE_STATUS_BAR_KEY] ?: true }
    override val isTopBarCollapsible: Flow<Boolean> =
        appContext.vaultDataStore.data.map { it[COLLAPSE_TOP_BAR_KEY] ?: true }
    override val isTabBarCollapsible: Flow<Boolean> =
        appContext.vaultDataStore.data.map { it[COLLAPSE_TAB_BAR_KEY] ?: true }
    override val isSecureContentEnabled: Flow<Boolean> =
        appContext.vaultDataStore.data.map { it[SECURE_CONTENT_KEY] ?: true }
    override val isFlipToLockEnabled: Flow<Boolean> =
        appContext.vaultDataStore.data.map { it[FLIP_TO_LOCK_KEY] ?: false }
    override val isFlipExitAndClearStackEnabled: Flow<Boolean> =
        appContext.vaultDataStore.data.map { it[FLIP_EXIT_AND_CLEAR_STACK_KEY] ?: false }
    override val cardStyle: Flow<VaultCardStyle> = appContext.vaultDataStore.data.map { prefs ->
        val globalStyle =
            parseStyleMap(prefs[CARD_STYLE_MAP_KEY])[DEFAULT_STYLE_KEY] ?: VaultCardStyle.fromKey(
                prefs[CARD_STYLE_KEY]
            )
        VaultCardStyle.normalizeGlobalStyle(globalStyle)
    }
    override val cardStyleByEntryType: Flow<Map<Int, VaultCardStyle>> =
        appContext.vaultDataStore.data.map { prefs ->
    ...
    override val autofillUiMode: Flow<AutofillUiMode> =
        appContext.vaultDataStore.data.map { prefs ->
        AutofillUiMode.fromKey(prefs[AUTOFILL_UI_MODE_KEY])
    }
    override val backupDirectoryUri: Flow<String?> = appContext.vaultDataStore.data.map { prefs ->
    ...
    override val visibleVaultTabs: Flow<Set<String>?> =
        appContext.vaultDataStore.data.map { prefs ->
    ...
    override val isDarkMode: Flow<Boolean?> =
        appContext.vaultDataStore.data.map { it[DARK_MODE_KEY] }
    ...
    override val isSwipeEnabled: Flow<Boolean> =
        appContext.vaultDataStore.data.map { it[SWIPE_ENABLED_KEY] ?: true }
    override val swipeLeftAction: Flow<SwipeActionType> = appContext.vaultDataStore.data.map {
        SwipeActionType.fromString(it[SWIPE_LEFT_ACTION_KEY] ?: SwipeActionType.COPY_PASSWORD.name)
    }
    override val swipeRightAction: Flow<SwipeActionType> = appContext.vaultDataStore.data.map {
    ...
    override suspend fun setStatusBarAutoHide(autoHide: Boolean) {
        appContext.vaultDataStore.edit { it[AUTO_HIDE_STATUS_BAR_KEY] = autoHide }
    }

    override suspend fun setTopBarCollapsible(collapsible: Boolean) {
        appContext.vaultDataStore.edit { it[COLLAPSE_TOP_BAR_KEY] = collapsible }
    }

    override suspend fun setTabBarCollapsible(collapsible: Boolean) {
        appContext.vaultDataStore.edit { it[COLLAPSE_TAB_BAR_KEY] = collapsible }
    }

    override suspend fun setSecureContentEnabled(enabled: Boolean) {
        appContext.vaultDataStore.edit { it[SECURE_CONTENT_KEY] = enabled }
    }

    override suspend fun setFlipToLockEnabled(enabled: Boolean) {
        appContext.vaultDataStore.edit { it[FLIP_TO_LOCK_KEY] = enabled }
    }

    override suspend fun setFlipExitAndClearStackEnabled(enabled: Boolean) {
        appContext.vaultDataStore.edit { it[FLIP_EXIT_AND_CLEAR_STACK_KEY] = enabled }
    }

    override suspend fun setCardStyle(style: VaultCardStyle) {
        appContext.vaultDataStore.edit { prefs ->
    ...
    override suspend fun setAutofillUiMode(mode: AutofillUiMode) {
        appContext.vaultDataStore.edit { it[AUTOFILL_UI_MODE_KEY] = mode.key }
    }

    override suspend fun setBackupDirectoryUri(uri: String) {
        appContext.vaultDataStore.edit { it[BACKUP_DIRECTORY_URI_KEY] = uri }
    }

    override suspend fun clearBackupDirectoryUri() {
        appContext.vaultDataStore.edit { it.remove(BACKUP_DIRECTORY_URI_KEY) }
    }

    override suspend fun setLastBackupExportFileName(fileName: String) {
        appContext.vaultDataStore.edit { it[LAST_BACKUP_EXPORT_FILE_NAME_KEY] = fileName }
    }
